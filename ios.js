...
function previewDocument(path:string, scheme:string) {
  if(Platform.OS === 'ios')
    return RNFetchBlob.previewDocument(path.startsWith('file://')? path : 'file://' + path, scheme)
  else
...
function openDocument(path:string, scheme:string) {
  if(Platform.OS === 'ios')
    return RNFetchBlob.openDocument(path.startsWith('file://')? path : 'file://' + path, scheme)
  else
